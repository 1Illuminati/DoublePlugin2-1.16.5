...
public class PropertiesInv extends InventoryManager {
	...
	public void click(InventoryClickEvent event) {
		...
		switch (pro) {
			case BAN_ITEM_REGISTER -> player.openInventory(BanItemAddInv.getInventory(player));
			case BAN_ITEM_SYSTEM -> player.openInventory(BanItemInfoInv.getInventory(player));
			case START_ITEM -> player.openInventory(StartItemInv.getInventory(player));
			default -> {
	...
	public Inventory getInv(NewPlayer player) {
		...
		invMaker.setItem(2, temp(PropertiesEnum.ATK_SPEED, Material.IRON_SWORD, "서버에 입장시 모든 유저는 기존 마크의 공속이 제거됩니다"));
		...
		invMaker.setItem(6, temp(PropertiesEnum.FALL_DAMAGE, Material.DIAMOND_BOOTS, "낙뎀을 받지 않습니다"));
